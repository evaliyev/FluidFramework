...
class NoIndexPaging {
  ...
  static async doPaging(query, changeSet, limitedPaths) {
    ...
    let memIndex = await NoIndexPaging._buildIndex(
      tokenizedPagingPath, tokenizedOrderClauses, query.from[0].depthLimit, changeSet
    );
  ...
  static async _buildIndex(tokenizedPagingPath, tokenizedOrderClauses, depthLimit, changeSet) {
    ...
    const _getPath = (context, length) => context._parentStack
      .slice(0, length)
      .map(PathHelper.quotePathSegmentIfNeeded)
      .join('.');
    ...
    return new Promise((resolve, reject) => {
      ...
      PropertyUtils.traverseChangeSetRecursivelyAsync(changeSet, {
        preCallback: (context, cb) => {
          if (ScanTraversalUtils.shouldStopTraversing(tokenizedPagingPath, subPaths, depthLimit, context)) {
            return cb('break');
          }
          ...
          if (ScanTraversalUtils.isItemContext(tokenizedPagingPath, depthLimit, context)) {
            if (!idx[_getPath(context, context._parentStack.length)]) {
              idx[_getPath(context, context._parentStack.length)] = Array(tokenizedOrderClauses.length).fill({
                value: undefined,
                typeId: undefined
              });
            }
          ...
          const orderClauseIndex = NoIndexPaging._getOrderClauseIndex(
            tokenizedPagingPath, tokenizedOrderClauses, depthLimit, context
          );
          ...
          if (orderClauseIndex > -1) {
            try {
              ...
              knownSortTypes[orderClauseIndex] = NoIndexPaging._validateSameType(
                knownSortTypes[orderClauseIndex], context.getTypeid()
              );
            } catch (ex) {
              reject(ex);
              return undefined;
            ...
            idx[_getPath(context, -1 * tokenizedOrderClauses[orderClauseIndex].by.length)][orderClauseIndex] = {
  ...
  static _validatePrimitiveType(type) {
    if (!TypeIdHelper.isPrimitiveType(type)) {
      throw new OperationError(`Attempting to perform paging on a non-primitive orderBy, type was ${type}`,
       '_validatePrimitiveType', HTTPStatus.BAD_REQUEST, OperationError.FLAGS.QUIET);
    }
  }
  ...
  static _validateSameType(knownType, type) {
    if (!knownType) {
      ...
      throw new OperationError(`Attempting to perform sorting on different types for orderBy ${type}/${knownType}`,
        '_validateSameType', HTTPStatus.BAD_REQUEST, OperationError.FLAGS.QUIET);
    }
  }
}
